...
public class StickerPackListAdapter extends RecyclerView.Adapter<StickerPackListItemViewHolder> {
    ...
    private void setAddButtonAppearance(ImageView addButton, StickerPack pack) {
        if (pack.getIsWhitelisted()) {
           //para cambiar el sticker a cuando ya lo agregue
            addButton.setImageResource(R.drawable.ic_action_name3);
            addButton.setClickable(false);
            addButton.setOnClickListener(null);
            setBackground(addButton, null);
        } else {

            //para cambiar el sticker cuando no se agregue

            addButton.setImageResource(R.drawable.ic_action_name); //aqui cambiaremos al boton de animacion
